...
librewolf-etp =
    .label = 增强型跟踪保护
    .description = LibreWolf 支持并默认启用了“严格”模式下的增强型跟踪保护。这是该浏览器最重要的设置之一，因为它提供了状态分区（State Partitioning）、严格的拦截列表以及其他出色的隐私功能。我们不建议将其更改为其他模式。
librewolf-swap-settings = 切换设置页面样式
    .title = 切换设置页面样式
librewolf-mouse-heading2 =
    .label = 鼠标行为
librewolf-JXL =
    .label = 启用 JXL (JPEG XL) 支持
permissions-canvas2 =
    .label = 提取 Canvas 画布数据
librewolf-is-default-browser-2 =
    .message = { -brand-short-name } 已设置为你的默认浏览器。
librewolf-is-not-default-browser-2 =
    .message = { -brand-short-name } 未设为您的默认浏览器。
